...
function sha2565(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = sha2564(from5(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes2(bytes);
}
...
function weierstrassN(CURVE, curveOpts = {}) {
  ...
  function pointFromBytes(bytes) {
    ...
    if (length === LC && (head === 2 || head === 3)) {
      ...
      const y2 = Fp3.fromBytes(tail.subarray(L * 1, L * 2));
      if (!isValidXY(x, y2))
  ...
  class Point3 {
    ...
    static fromHex(hex) {
      const P = Point3.fromAffine(fromBytes7(ensureBytes("pointHex", hex)));
      P.assertValidity();
      return P;
    }
    ...
    toBytes(isCompressed = true) {
      abool("isCompressed", isCompressed);
      this.assertValidity();
      return toBytes8(Point3, this, isCompressed);
    }
  ...
  Point3.Fn = Fn;
  const bits = Fn.BITS;
  const wnaf = new wNAF(Point3, curveOpts.endo ? Math.ceil(bits / 2) : bits);
  return Point3;
}
...
function ecdsa(Point3, hash, ecdsaOpts = {}) {
  ...
  class Signature {
    ...
    recoverPublicKey(msgHash) {
      ...
      const R = Point3.fromHex(concatBytes(pprefix((rec & 1) === 0), x));
  ...
  function isValidPublicKey(publicKey, isCompressed) {
    try {
      ...
      if (isCompressed === true && l2 !== lengths.public)
        return false;
      if (isCompressed === false && l2 !== lengths.publicUncompressed)
        return false;
  ...
  function isProbPub(item) {
    ...
    return l2 === lengths.public || l2 === lengths.publicUncompressed;
  }
  function getSharedSecret(secretKeyA, publicKeyB, isCompressed = true) {
    ...
    const b2 = Point3.fromHex(publicKeyB);
...
function createHasher4(Point3, mapToCurve, defaults) {
...
function lift_x(x) {
  ...
  if (!hasEven(y2))
    y2 = modP(-y2);
  const p = Point.fromAffine({ x, y: y2 });
  p.assertValidity();
  return p;
}
...
function fromDerHex(signature) {
  const { r, s } = secp256k12.Signature.fromDER(from4(signature).slice(2));
  return { r, s };
}
  ...
  constructor({ signature }) {
    super(`Value \`${signature}\` is an invalid signature size.`, {
      metaMessages: [
        ...
        `Received ${size4(from4(signature))} bytes.`
      ]
    });
    Object.defineProperty(this, "name", {
      enumerable: true,
...
function signatureG1FromBytes(hex) {
  ...
  if (y2 * _2n7 / P !== aflag)
    y2 = Fp.neg(y2);
  const point = Point3.fromAffine({ x, y: y2 });
  point.assertValidity();
  return point;
}
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b2, from14, to) => bytesToNumberBE(b2.slice(from14, to));
...
function signatureG2FromBytes(hex) {
  ...
  if (isGreater || is0)
    y3 = Fp2.neg(y3);
  const point = Point3.fromAffine({ x, y: y3 });
  point.assertValidity();
  return point;
}
var bls12_381 = bls({
...
function createSWU2(Point3, opts) {
...
async function verify4(options) {
  ...
  return await globalThis.crypto.subtle.verify({
    name: "ECDSA",
    hash: "SHA-256"
  }, publicKey, concat5(fromNumber3(signature.r), fromNumber3(signature.s)), from5(payload));
}
...
function createWalletSendCallsRequest({ calls, from: from14, chainId, capabilities }) {
...
*/
//# sourceMappingURL=dist-QZVLZWON.js.map
